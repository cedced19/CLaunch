...
public class LoginForm extends TransparentPanel
{
  ...
  private TransparentButton launchButton = new TransparentButton("Connection");
  ...
  private JScrollPane getUpdateNews()
  {
    ...
    try
    {
      ...
      new Thread() {
          public void run() {
            try {
              editorPane.setPage(new URL(GameUpdater.urlNews));
            } catch (Exception e) {
              e.printStackTrace();
              editorPane.setText("Echec du chargement des news");
            }
          }
        }
      .start();
